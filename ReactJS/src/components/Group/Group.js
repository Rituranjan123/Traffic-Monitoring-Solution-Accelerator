...
import { UILabel, TableColoum, PageURL } from './typeofassests';
...
class Group extends Component {
  ...
  render() {
    const { Data, search, deleteModal, formOption } = this.state;
            ...
            {Data.map((item) => (
              <div className="listdata" key={item.ID}>
                {Object.keys(TableColoum).map((key) => (
                  <div className="other">{item[key]}</div>
                ))}

                <div className="action">
                  <div className="edit">
                    <Link to={{ pathname: PageURL.Update + item.GROUP_ID }}>
                      Edit
                    </Link>
                  </div>
                  ...
                  <div className="Authorization">
                    <Link
                      to={{ pathname: PageURL.Authorization + item.GROUP_ID }}
                    >
                      Authorization
                    </Link>
                  </div>
                  ...
                  MenuData[73].DELETE_RIGHT === true ? (
                    <div
                      className="delete"
                      onClick={() => this.handleDeleteModal(item.GROUP_ID)}
                    >
                      Delete
                    </div>
                  ) : (
